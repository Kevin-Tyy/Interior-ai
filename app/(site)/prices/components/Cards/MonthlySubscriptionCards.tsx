...
export default function MonthlySubscriptionCards({
  products,
  handleSubcription,
  selectedProduct,
  isPending,
            ...
            <div className="min-h-[650px] xl:min-h-[520px] w-full pb-[36px] pt-[86px] flex flex-col justify-between">
              <div className="space-y-8">
                <h1 className="text-5xl text-primary-black font-bold">
                  {products[0].unit_amount / 100} <span className="text-lg text-primary-grey">/month</span>
                </h1>
              ...
              <button
                ...
                {isPending && selectedProduct === products[0].id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Choose plan
              </button>
            </div>
            <div className="min-h-[650px] xl:min-h-[520px] w-full pb-[36px] pt-[86px] flex flex-col justify-between">
              <div className="space-y-8">
                <h1 className="text-5xl text-primary-black font-bold">
                  {products[1].unit_amount / 100} <span className="text-lg text-primary-grey">/month</span>
                </h1>
              ...
              <button
                disabled={isPending}
                onClick={() => {
                  setSelectedProduct(products[1].id);
                  handleSubcription(products[1]);
                }}
                className={clsx(
                  "w-full bg-primary-lightgray text-white p-4 rounded-full font-semibold hover:scale-105 transition duration-300 flex justify-center items-center",
                  isPending && "opacity-90"
                )}>
                {isPending && selectedProduct === products[1].id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Choose plan
              </button>
            </div>
            <div className="hidden xl:block bg-primary-orange text-primary-black px-10 pt-4 pb-10 rounded-[44.103px] w-full shadow-card">
              <div className="min-h-[520px] w-full flex flex-col justify-between">
                <div className="space-y-8">
                  ...
                  <h1 className="text-5xl text-primary-black font-bold">
                    {products[2].unit_amount / 100} <span className="text-lg font-semibold">/month</span>
                  </h1>
              ...
              <button
                disabled={isPending}
                onClick={() => {
                  setSelectedProduct(products[2].id);
                  handleSubcription(products[2]);
                }}
                className={clsx(
                ...
                {isPending && selectedProduct === products[2].id && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Choose plan
              </button>
            </div>
          </div>
